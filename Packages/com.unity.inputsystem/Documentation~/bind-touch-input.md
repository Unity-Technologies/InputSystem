...
If you bind a single action to input from multiple touches, set the action type to [pass-through](RespondingToActions.md#pass-through) so the action gets callbacks for each touch, instead of just one.
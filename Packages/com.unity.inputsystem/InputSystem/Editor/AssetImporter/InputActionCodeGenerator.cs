		...
		public static string GenerateWrapperCode(InputActionAsset asset, Options options = default)
		{
			...
			foreach(InputActionMap map in maps)
			{
				...
				for(int i = 0; i < map.actions.Count; i++)
				{
					...
					string actionName = CSharpCodeHelpers.MakeIdentifier(action.name);
					if(i < map.actions.Count - 1)
					{
			...
			writer.WriteLine($"// control scheme enums");
			writer.WriteLine($"public enum ControlSchemeEnum");
			writer.BeginBlock();

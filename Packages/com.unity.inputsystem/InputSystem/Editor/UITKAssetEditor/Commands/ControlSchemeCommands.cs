...
namespace UnityEngine.InputSystem.Editor
{
    internal static class ControlSchemeCommands
    {
        ...
        public static Command ChangeControlSchemeForBinding(int selectedBindingIndex, string controlScheme, bool add)
        {
            return (in InputActionsEditorState state) =>
            {
                ...
                var groups = serializedProperty.FindPropertyRelative(nameof(InputBinding.m_Groups)).stringValue;
                var set = new SeparatedStringSet(groups, InputBinding.kSeparatorString);
                if (add)
                    set.Add(controlScheme);
                else
                    set.Remove(controlScheme);
                serializedProperty.FindPropertyRelative(nameof(InputBinding.m_Groups)).stringValue = set.Value;

                return state.With(); // TODO
            };
        }
    }
}

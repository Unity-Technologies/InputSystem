...
- Added a new high-performance way to iterate over changed controls in an event.
  ...
  * This can be used, for example, to implement much more performant "any button pressed?" queries.
  ...
  ```
  - Added Step Counter sensor for iOS, you need to enable **Motion Usage** under Input System settings before using the sensor. You can also manually add **Privacy - Motion Usage Description** to Info.plist file.

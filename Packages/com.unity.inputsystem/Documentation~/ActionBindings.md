...
* A binding path can also contain wildcards, such as `<Gamepad>/button*`. This will match any control on any gamepad with a name starting with "button", which will match all the four action buttons on any connected gamepad.

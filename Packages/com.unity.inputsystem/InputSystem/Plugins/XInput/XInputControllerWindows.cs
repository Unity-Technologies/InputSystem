...
namespace UnityEngine.InputSystem.XInput.LowLevel
{
    ...
    // Was an attempt to implement generic HID support for XBox One on Windows
    // The binary struct doesn't match the actual device, so it doesn't work here
    [StructLayout(LayoutKind.Explicit)]
    internal struct XInputControllerWindowsHIDState : IInputStateTypeInfo
    {
        ...
        public XInputControllerWindowsHIDState WithButton(Button button)
        {
            Debug.Assert((int)button < 16, $"Expected button < 16, so we fit into the 16 bit wide bitmask");
            buttons |= (ushort)(1U << (int)button);
            return this;
...
namespace UnityEngine.InputSystem.XInput
{
    ...
    [InputControlLayout(displayName = "Xbox Controller", stateType = typeof(XInputControllerWindowsHIDState), hideInUI = true)]
    public class XboxGamepadWindowsHID : XInputController
    {
    }

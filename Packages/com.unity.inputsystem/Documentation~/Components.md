...
The component must be on the same `GameObject` to use `Send Messages`, or on the same or any child `GameObject` to use `Broadcast Messages`.
...
Once you've completed this setup, when the `PlayerInput` component configures the Actions for a specific player, it assigns the same Action configuration to the `InputSystemUIInputModule`. In other words, the same Action and Device configuration used to control the player now also controls the UI.

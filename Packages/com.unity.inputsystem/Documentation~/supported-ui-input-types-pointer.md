...
Because multiple pointer devices can feed into the same set of actions, it's important to set the [action type](./respond-to-input.md#action-types) to [PassThrough](xref:UnityEngine.InputSystem.InputActionType). This ensures that no filtering is applied to input on these actions and that instead every input is relayed as is.

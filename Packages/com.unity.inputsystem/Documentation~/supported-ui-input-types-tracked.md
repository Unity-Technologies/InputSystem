...
> [!IMPORTANT]
> Because multiple tracked devices can feed into the same set of actions, it is important to set the [action type](./respond-to-input.md#action-types) to [PassThrough](xref:UnityEngine.InputSystem.InputActionType). This ensures that the Input System applies no filtering on these input actions, and relays inputs correctly.

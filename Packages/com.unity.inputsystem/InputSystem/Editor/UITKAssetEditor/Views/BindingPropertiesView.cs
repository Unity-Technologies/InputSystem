...
namespace UnityEngine.InputSystem.Editor
{
    internal class BindingPropertiesView : ViewBase<BindingPropertiesView.ViewState>
    {
        ...
        public override void RedrawUI(ViewState viewState)
        {
            ...
            {
                var availableControlSchemesContainer = m_Root.Q<VisualElement>("available-control-schemes-container");
                foreach (var controlScheme in viewState.controlSchemes)
                {
                    var checkbox = new Toggle(controlScheme.name);
                    checkbox.value = binding.controlSchemes.Any(scheme => controlScheme.name == scheme);
                    availableControlSchemesContainer.Add(checkbox);
                    checkbox.RegisterValueChangedCallback((changeEvent) => {
                        Dispatch(ControlSchemeCommands.ChangeControlSchemeForBinding(
                            viewState.selectedBindingIndex, controlScheme.name, checkbox.value));
                    });
                }
            }
        }
